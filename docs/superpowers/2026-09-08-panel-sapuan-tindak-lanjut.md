...
| 6 | **DITUTUP** (`a5c3903`). `aria-pressed` pada primitif `src/app/_shell/panel/bilah-daftar.tsx:78` — tidak sah di atas `<Link>`, yang bukan `role="button"`/`switch`, dan karena itu menyala secara identik (hanya lewat warna, tanpa tanda bagi pembaca layar) di ketujuh halaman pemakainya — diganti `aria-current={menyala ? "true" : undefined}`, menyamai pola yang sudah dipakai `sidebar.tsx`/`bottom-bar.tsx`. Dibuktikan lewat `tests/panel-bilah-daftar.test.tsx` (RED dengan `aria-pressed` dikembalikan sementara, GREEN sesudah dipulihkan). **Ini menutup HANYA soal atribut ARIA-nya.** Masalah visual yang disebut baris ini sebagai konteks (kelompok chip berdampingan tanpa penanda batas) adalah utang **#5**, baris TERPISAH — tidak tersentuh perbaikan ini dan TETAP TERBUKA: tujuh halaman masih menampilkan chip berdampingan tanpa penanda kelompok, `/admin/sesi` masih tiga kelompok sekaligus tanpa jeda visual. | Tidak lagi ditunda — sudah selesai untuk atribut ARIA-nya. Utang #5 (label kelompok tidak dirender) tetap menunggu keputusan tata letak, sama seperti sebelumnya. |

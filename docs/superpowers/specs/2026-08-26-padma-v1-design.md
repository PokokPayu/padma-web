...
- **Pencocokan identitas wajib operator setara (`=`), tidak pernah pola.** Penautan akun klien pernah memakai `.ilike("email", …)` yang oleh PostgREST menjadi SQL LIKE: cukup mendaftar mandiri dengan email `%@padma.test` untuk menautkan akun penyerang ke baris klien orang lain berikut catatan sesinya (data kesehatan, UU PDP). Perbaikannya: email dinormalkan (trim + huruf kecil) saat disimpan — ditegakkan trigger + constraint DB — lalu dicocokkan dengan `.eq()`, sehingga beda kapitalisasi tetap tertaut tapi `%`/`_` tidak lagi menjadi wildcard. Berlaku untuk setiap pencarian identitas berikutnya (email, PADMA ID, kode skrining).
- `enable_signup` sengaja **tetap menyala**: diuji 28 Agu 2026, mematikannya menolak login Google pertama kali (`signup_disabled`) sehingga aktivasi akun klien (keputusan #4) mati; pembuatan user oleh admin lewat service role tetap jalan. Perisai kebocoran adalah pencocokan email persis di atas, bukan tertutupnya pendaftaran.
